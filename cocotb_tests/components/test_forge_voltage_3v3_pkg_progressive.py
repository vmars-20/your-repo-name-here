...
def voltage_to_digital_3v3(voltage):
    """Convert voltage to digital value for 3.3V domain"""
    # Scale: 0V → 0, 3.3V → 32767
    if voltage < 0.0:
        voltage = 0.0
    elif voltage > 3.3:
        voltage = 3.3
    return int((voltage / 3.3) * 32767.0 + 0.5)


def digital_to_voltage_3v3(digital):
    """Convert digital value to voltage for 3.3V domain"""
    # Scale: 0 → 0V, 32767 → 3.3V
    return (digital / 32767.0) * 3.3
...
async def test_p1_t1_to_digital_accuracy(dut):
    ...
    if VERBOSITY != "SILENT":
        dut._log.info("=" * 70)
        dut._log.info("P1 - BASIC TESTS (forge_voltage_3v3_pkg)")
    ...
    test_cases = [
        (0.0, 0),         # 0V → 0
        (1.65, 16384),    # Mid-point (half of 3.3V)
        (3.3, 32767),     # Max voltage
    ]

    for voltage, expected_digital in test_cases:
        # Set test voltage (as digital for wrapper)
        voltage_digital = voltage_to_digital_3v3(voltage)
...
async def test_p1_t2_from_digital_roundtrip(dut):
    ...
    for test_digital in test_digitals:
        ...
        if VERBOSITY in ["VERBOSE", "DEBUG"]:
            voltage = digital_to_voltage_3v3(test_digital)
...
async def test_p1_t4_clamp_behavior(dut):
    ...
    test_cases = [
        ...
        (1.65, 1.65),   # Mid (no clamp)
        (3.3, 3.3),     # Max (no clamp)
    ]

    for voltage, expected_clamped in test_cases:
        voltage_digital = voltage_to_digital_3v3(voltage)
        expected_digital = voltage_to_digital_3v3(expected_clamped)
...
async def test_p2_t5_precision_tests(dut):
    ...
    test_voltages = [0.0, 0.5, 1.0, 1.8, 2.5, 3.0, 3.3]
    ...
    for voltage in test_voltages:
        ...
        expected = voltage_to_digital_3v3(voltage)
...
async def test_p2_t6_reference_voltages(dut):
    ...
    test_cases = [
        (1.0, 9930),    # DIGITAL_1V0
        (1.8, 17874),   # DIGITAL_1V8
        (2.5, 24825),   # DIGITAL_2V5
        (3.0, 29790),   # DIGITAL_3V1
        (3.3, 32767),   # DIGITAL_3V3
    ]
